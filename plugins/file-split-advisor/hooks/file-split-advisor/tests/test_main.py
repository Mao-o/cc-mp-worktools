...
class BaseMainTest(unittest.TestCase):
    ...
    def _write_nested(self, rel: str, content: str) -> Path:
        """``self.tmp`` 配下にディレクトリごと作ってファイルを書く。

        ディレクトリ名に依存する判定 (test role / 第三者ディレクトリ skip) は
        実在するパスでないと ``source.load_text`` まで到達しないため、
        フラットな ``_write`` とは別に用意する。
        """
...
class TestSkipPatterns(BaseMainTest):
    ...
    def test_third_party_dir_skipped_end_to_end(self):
        # ディレクトリ名 skip (0.4.0 新設) が __main__ 経由でも効くこと。
        path = self._write_nested("vendor/lib/big.py", _python_lines(900))
        out, _ = _run_main(self._envelope(path))
        self.assertEqual(out, "")

    def test_generated_marker_after_a_long_license_header_skipped(self):
        # 走査幅 20 行の理由そのもの: 10〜15 行のライセンスヘッダの後に生成物
        # 注記を置く生成器 (OpenAPI Generator 等) を取りこぼさないこと。
        header = "".join(f"# license line {i}\n" for i in range(12))
        content = header + "# Code generated by openapi-generator. DO NOT EDIT.\n"
        path = self._write("api_client.py", content + _python_lines(900))
        out, _ = _run_main(self._envelope(path))
        self.assertEqual(out, "")


class TestTestRoleDirScoping(BaseMainTest):
    """test ディレクトリ判定を cwd からの相対部分に限定した効果 (0.4.0)。"""
    ...
    def test_relative_test_dir_still_gets_the_test_role(self):
        # 相対部分に `tests` があれば従来どおり test role (床テスト)。
        cwd = str(Path(self.tmp) / "test" / "project")
        path = self._write_nested("test/project/tests/utils.py", _python_lines(900))
        envelope = self._envelope(path)
        envelope["cwd"] = cwd
        out, _ = _run_main(envelope)
        text = self._context(out)
        self.assertIn("test: 閾値", text)
