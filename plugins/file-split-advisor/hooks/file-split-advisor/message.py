...
def _format_multiplier(value: float) -> str:
    ...
    表現」を返す (David Gay のアルゴリズム)。丸めによる情報損失が原理的に
    起こらないため、以後どんな値が来ても「表示された係数から実効閾値を
    導出できる」という不変条件を保てる。既定の言語/role/宣言的緩和の係数は
    もともと2桁以内の値なので、この変更で既存の表示は変わらない。float は
    ``str()`` が常に小数点を含む (例: ``str(2.0) == "2.0"``)ため、以前の
    「末尾に .0 を補う」後処理も不要になった。
    """
