...
import、副作用専用で `core` / `hooks` を sys.path に通す) が
`ModuleNotFoundError` になる。ディレクトリ探索 (`python3 -m unittest discover
tests`) は探索先自体を検索パスに入れるため問題にならず、この非対称が「テストが
壊れている」と誤認されやすい (内部バックログ)。
