...
class TestGlobalDefault(BaseAncestorBoundary):
    ...
    プロジェクト側で何も見つからないときだけ使う明示的な fallback (v0.13.0)。
    親遡及は従来どおり `$HOME` を越えない — 位置依存の継承を復活させないため。
    """
    ...
    def setUp(self):
        ...
        self._set_home(self.home)
        self.project = self.tmp / "workspace" / "project"
        self.project.mkdir(parents=True)

    def _resolve(self, start: Path):
        return paths.resolve_accounts_file_for_verification(str(start))

    def test_global_accounts_file_points_at_new_path_under_home(self):
        self.assertEqual(
            paths.global_accounts_file(),
            self.home / ".claude" / "verify-cloud-account" / "accounts.local.json",
        )

    def test_falls_back_to_global_when_project_has_nothing(self):
        expected = self._write_new(self.home, {"github": "global-user"})
        path, kind, conflicts, resolved_dir, source = self._resolve(self.project)
    ...
    def test_project_file_wins_over_global(self):
        ...
        project_file = self._write_new(self.project, {"github": "project-user"})
        path, _kind, _conflicts, _resolved, source = self._resolve(self.project)
        self.assertEqual(path, project_file)
        self.assertEqual(source, paths.SOURCE_PROJECT)

    def test_project_conflicts_do_not_fall_back_to_global(self):
        """同一階層の tier 競合は fail-closed のまま (グローバルで救済しない)。"""
        self._write_new(self.home, {"github": "global-user"})
        self._write_new(self.project, {"github": "a"})
        self._write_legacy(self.project, {"github": "b"})
        path, _kind, conflicts, _resolved, source = self._resolve(self.project)
        self.assertIsNone(path)
        self.assertEqual(len(conflicts), 2)
        self.assertIsNone(source)

    def test_legacy_global_paths_are_not_used(self):
        """`~/.claude/accounts.json` などの旧名はグローバル既定にしない。

        親遡及が `$HOME` まで届いていた頃に無関係な `~/.claude/accounts.json` を
        継承していたのが塞いだ不具合そのもの。旧名を認めると再現してしまう。
        """
        self._write_legacy(self.home, {"github": "global-legacy"})
        (self.home / ".claude" / "accounts.local.json").write_text(
            json.dumps({"github": "global-deprecated"}), encoding="utf-8"
        )
        path, _kind, conflicts, _resolved, source = self._resolve(self.project)
        self.assertIsNone(path)
        self.assertEqual(conflicts, [])
    ...
    def test_builder_resolution_does_not_fall_back_to_global(self):
        """builder (書込先の決定) はグローバル既定へ落ちない。

        落ちると「プロジェクト設定を作るつもりの編集」が全プロジェクトに効く
        ファイルを書き換えてしまう。
        """
        self._write_new(self.home, {"github": "global-user"})
        path, kind, conflicts, resolved_dir = paths.resolve_accounts_file(
            str(self.project)
        )
        self.assertIsNone(path)
        self.assertIsNone(kind)
        self.assertEqual(conflicts, [])
        self.assertIsNone(resolved_dir)

    def test_ancestor_search_still_stops_before_home(self):
        """遡及そのものは `$HOME` を越えない (専用経路だけがグローバルを読む)。"""
        project_in_home = self.home / "dev" / "proj"
        project_in_home.mkdir(parents=True)
        self._write_legacy(self.home, {"github": "global-legacy"})
        self.assertIsNone(self._resolved_dir(project_in_home))
