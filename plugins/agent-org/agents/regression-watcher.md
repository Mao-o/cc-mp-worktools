...
description: |
  ...
  記録する。bd は **hard dependency** (`~/.beads/<proj-hash>/.beads/` が
  未初期化なら即座に abort)。修正は regression-fixer に委譲する分業設計。
memory: user
...
v0.6.0 から detection 永続化は **beads (bd CLI) が hard dependency**。以下を
...
# 2. proj-hash 計算 (cwd canonicalize + sha256 先頭 8 桁)
PROJ_HASH=$(python3 -c "
import hashlib, os
...
# 4. bd prime (subagent コンテキストに repo 状態と open issues を inject)
...
検出した regression を以下の形で `bd create` する (preflight で
`BEADS_DIR=~/.beads/<proj-hash>/.beads` を export 済の前提):

```bash
bd create "<observation summary 80 chars 以内>" \
  -t detection \
...
- bd standard type に `detection` は無く、`/org-init` で
  `bd config set types.custom "detection,fix,approval,episode"` を実行済の前提
...
bd list -t detection --status open --json \
  | jq -r '.[] | select(
