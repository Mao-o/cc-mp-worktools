...
description: |
  ...
  戻す設計。v0.6.0 から **beads (bd CLI) が hard dependency**。detection / fix
  state は `~/.beads/<proj-hash>/.beads/` に格納。
memory: user
...
v0.6.0 から fix state 永続化は **beads (bd CLI) が hard dependency**。以下を
...
# 3. ~/.beads/<proj-hash>/.beads/ 初期化済み確認
export BEADS_DIR="$HOME/.beads/$PROJ_HASH/.beads"
[ -d "$BEADS_DIR" ] || {
  echo "FATAL: $BEADS_DIR not initialized. Run /org-init from project root"; exit 1;
}

# 4. gh auth status (修正成果を push / gh pr 経由で main に戻すため必須)
gh auth status >/dev/null 2>&1 || {
  echo "FATAL: gh CLI not authenticated. Run 'gh auth login' from foreground"; exit 1;
}

# 5. bd prime (open detection / 関連 fix issues の状態をコンテキストに inject)
...
DETECTION_ID=$(bd ready -t detection --json \
...
FIX_ID=$(bd create "fix: $(bd show $DETECTION_ID --json | jq -r .title | head -c 60)" \
  -t fix \
...
bd dep add "$DETECTION_ID" "$FIX_ID"

# Step 4. fix を即 claim (audit-only 用途でも claim でロック明示)
bd update "$FIX_ID" --claim 2>/dev/null || true
```
...
1. **対象を理解する**
   - `bd show $DETECTION_ID` で description body (YAML schema) を読む
...
OLD_DESC="$(bd show "$DETECTION_ID" --json | jq -r .description)"
NEW_DESC="$(cat <<EOF
$OLD_DESC
...
bd update "$DETECTION_ID" -d "$NEW_DESC"
bd close "$DETECTION_ID"  # dep が無いので close 可能
```
...
bd update "$FIX_ID" -d "<上記 schema、goal_status: error、notes に停止理由>" \
  -l "outcome:error"
bd close "$FIX_ID"
# detection は open のまま (dep は外れているので別 fixer が後で再 claim 可能)
```

main session が `bd list -t fix -l outcome:error --json` で error 終了した
fix を確認し、人間判断ルートに戻す。
