...
description: |
  ...
  `.claude/agent-org/approvals/<task-id>.json` への書込は呼び出し側 command
  (`/run-review`) の責務 (このスキルは spawn と verdict 回収まで)。
...
- approval workflow に乗せて (`/run-review` 経由で approval JSON を書く)
  task の quality gate を効かせたい
...
- `<task-id>`: 後で approval JSON を書く際の識別子
...
approval JSON への書込は呼び出し側 command が行う (このスキルは行わない)。
...
- **reviewer subagent は真 RO** (Read/Glob/Grep のみ)。verdict はファイル書込
  しないため、approval JSON 書込は必ず呼び出し側 command で行う
...
- 上位 command: `commands/run-review.md` (verdict 集約 + approval JSON 書込)
