...
- `<repo>/.beads/` 初期化済 (v0.8.0 から repo-local 配置、未なら `/org-init` を先に実行)
...
APPROVALS_DIR="$REPO_ROOT/.claude/agent-org/approvals"
LEGACY_DIR="$REPO_ROOT/.claude/agent-org/approvals.legacy"
...
- approval 発行元: `skills/running-review/SKILL.md` (v0.7.0+ で bd issue 化)
