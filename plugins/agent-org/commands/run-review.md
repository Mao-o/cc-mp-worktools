...
`task-completed-gate.sh` (Phase 3 で追加) は **approval JSON opt-in 設計** で
動く。task が TaskCompleted を発火した時、hook は top-level `task_id` を取り
出して `.claude/agent-org/approvals/<task_id>.json` を探す:

- approval JSON **不在** → pass (通常 task、`/run-review` を回していない)
- `approval_status=approved`: gate pass
- `approval_status=conditional`: gate pass (warn message のみ出力)
- `approval_status=rejected`: gate block (exit 2)

公式 TaskCompleted hook schema は全 field が top-level フラット
(`task_id` / `task_subject` / `task_description` / `teammate_name` /
`team_name`) で `task` / `metadata` / `review_required` などの nested 設定
field は存在しない。そのため「review が必要な task」を明示する schema 上の
方法は無く、approval JSON を作るか否かで opt-in を表現する設計に倒している。
