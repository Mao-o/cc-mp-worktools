...
v0.7.0 から approval は `~/.beads/<proj-hash>/.beads/` 上の bd issue に一本化
(旧 `.claude/agent-org/approvals/<task-id>.json` は廃止、`/migrate-approvals-to-beads`
で変換可能)。
...
- `~/.beads/<proj-hash>/.beads/` 初期化済 (未なら `/org-init` 先に実行)
...
2. **task issue を find-or-create する** (Phase 6.1.0 規約)
   ```bash
   export BEADS_DIR="$HOME/.beads/$(python3 -c '
   import hashlib, os
   print(hashlib.sha256(os.path.realpath(os.getcwd()).encode()).hexdigest()[:8])
   ')/.beads"
...
6. **dep を張る** — `bd dep add <task> <approval>`
   ...
   bd dep add "$task_bd" "$appr_bd"
   ```
   - approved (priority=2) の場合は dep 作成後すぐ `bd close "$appr_bd"` で
     blocker 解除
...
rejected_count="$(bd list -l "task:${task_id}" -t approval --status open --json \
...
1. `bd list -l "task:${task_id}" -t approval --status open --json | jq -r '.[].id'`
...
3. **supersedes dep を張る** — bd 1.0.4 で `--type supersedes` 公式サポート (U13 検証済):
   ...
   for old_appr in $existing_approvals; do
     bd dep add "$new_appr" "$old_appr" --type supersedes
     bd close "$old_appr"
   done
   ```
...
- approval は bd 経由で `~/.beads/<proj-hash>/.beads/` (working dir 外) に書込
- worktree 隔離の影響を受けない (cross-session 共有 state)
