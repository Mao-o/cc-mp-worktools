...
> AI CLI へ送ってしまう方向 (送信範囲が広がる方向) には倒さない。**commit した後
> 同じターンで push まで済ませた場合も証明できないほうに該当する** (push した
> 時点でその commit は `refs/remotes/origin/*` から到達可能になり、`--not
> --remotes` の対象から外れるため) — 「直して push まで」を 1 ターンでやると
> 復元ではなく通知止まりになる。復元できなかったパスは黙って消費せず、
> `systemMessage` に「差分が空で取得できませんでした (commit 済みの可能性)」として
> 列挙し、次ターンに繰り返し報告しないよう pending からは外す。
