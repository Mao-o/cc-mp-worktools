...
> **同一ターン内で commit した変更の扱い** (0.11.0 で復元できるようになった):
> 変更を記録してから Stop までの間にそのファイルを commit すると、HEAD 基準の
> diff は空になる。0.11.0 は **「その内容を書いたのは自分だ」と証明できたときだけ**
> その commit の差分をレビューに使う:
>
> - PostToolUse が編集直後にファイル内容の指紋 (sha256) を記録する
>   (**内容そのものは保存しない**。git も呼ばないので PostToolUse は軽いまま)
> - Stop 時点のディスク内容がその指紋と一致するときだけ復元する。HEAD 基準 diff が
>   空 = ディスクと HEAD が一致しているので、指紋一致はそのまま「HEAD に入っている
>   内容 = 自分が書いた内容」を意味する
> - 基点は**そのパスを最後に変更した commit の親**。送る差分は「commit が挟まらな
>   かった場合に送っていた差分」と同じ範囲で、それ以上遡らない。衝突解決を伴う
>   merge がそのパスの内容を決めている場合 (親が 2 つ) は復元しない
> - 復元したターンは `systemMessage` に「commit の差分をレビューに使用」と出す
> - **証明できない場合は復元せず**、`systemMessage` に「差分が空で取得できません
>   でした」として列挙し、次ターンに繰り返し報告しないよう pending からは外す
>   (黙って消費しない)。別のローカルの書き手 (別セッション・人間の手動 commit) が
>   同じパスへ commit した場合はここに落ちる
> - **追跡ファイルの削除が同一ターン内で commit された場合**も同じ枠で扱う
>   (指紋は「ディスクに無かった」ことも表せるので、削除の差分としてレビューされる)。
>   一度も commit されていない一時ファイルは基点が無いので復元されず、通知だけが出る
>
> 0.9.0 は「前回 Stop が記録した HEAD」を基点にして `git rev-list --not --remotes` で
> 「手元由来」(どのリモートにも存在しない) と証明できれば遡った diff を送っていたが、
> この証明は「リモート由来でない」ことしか示しておらず「このセッションが書いた」ことは
> 示していない。同一 worktree を共有する別のローカルの書き手が push せずに同じパスへ
> commit した内容が丸ごと外部 AI CLI へ送られることがマージ前レビューで実演されたため、
> 0.10.0 で復元経路そのものを撤去した (通知のみ)。0.11.0 は基点ではなく**内容**で
> 帰属を証明する形で復元を戻したもので、**送信範囲が広がる方向には倒さない**方針は
> 変わらない (証明できなければ送らない)。
