...
class TestContentDigest(ReviewSetTestCase):
    """内容指紋 (`content_digest` / `_content_digests`) の境界。

    指紋は「HEAD 基準 diff が空になったパスを復元してよいか」の唯一の証明材料なので、
    **取れなかったときは必ず None** (呼び出し側で復元しない = 送らない側) になること。
    """

    def test_regular_file_digest_matches_sha256(self):
        path = write(self.repo, "a.py", "print(1)\n")
        expected = hashlib.sha256(b"print(1)\n").hexdigest()
        self.assertEqual(self.entry.content_digest(path), f"sha256:{expected}")

    def test_missing_file_is_absent_marker(self):
        path = os.path.join(self.repo, "gone.py")
        self.assertEqual(self.entry.content_digest(path), self.entry.DIGEST_ABSENT)

    def test_oversized_file_has_no_digest(self):
        path = write(self.repo, "big.py", "x" * 100)
        with mock.patch.object(self.entry, "SNAPSHOT_MAX_FILE_BYTES", 10):
            self.assertIsNone(self.entry.content_digest(path))

    def test_directory_has_no_digest(self):
        os.makedirs(os.path.join(self.repo, "dir"))
        self.assertIsNone(self.entry.content_digest(os.path.join(self.repo, "dir")))

    def test_fifo_does_not_block(self):
        """FIFO を `open()` すると書き手が来るまで戻らない。`os.stat` で種別を見て
        弾いているので、10 秒の PostToolUse hook をハングさせない。

        **daemon thread + join(timeout) で測る**: 退行すると `content_digest` が
        永久にブロックするため、直接呼ぶとテストがハングして CI が「失敗」ではなく
        「無応答」になる (signal はプラットフォーム依存なので使わない)。
        """
        path = os.path.join(self.repo, "pipe")
        os.mkfifo(path)
        result: list = []
        worker = threading.Thread(
            target=lambda: result.append(self.entry.content_digest(path)), daemon=True
        )
        worker.start()
        worker.join(timeout=5)
        self.assertFalse(
            worker.is_alive(), "FIFO の指紋取得がブロックしている (open してはいけない)"
        )
        self.assertEqual(result, [None])

    def test_batch_stops_at_total_budget(self):
        first = write(self.repo, "one.py", "a" * 100)
        second = write(self.repo, "two.py", "b" * 100)
        with mock.patch.object(self.entry, "SNAPSHOT_BUDGET_BYTES", 50):
            digests = self.entry._content_digests([first, second])
        self.assertIn(first, digests)
        self.assertNotIn(second, digests, "合計予算を超えた以降は指紋を記録しない")

    def test_batch_skips_unhashable_paths(self):
        path = write(self.repo, "a.py", "print(1)\n")
        os.makedirs(os.path.join(self.repo, "dir"))
        digests = self.entry._content_digests([os.path.join(self.repo, "dir"), path])
        self.assertEqual(list(digests), [path])


class TestRestoreCommittedDiff(ReviewSetTestCase):
    """同一ターン内 commit の差分復元 (`_restore_committed_diff`) の判定。

    復元してよいのは「Stop 時点のディスク内容が、編集直後に記録した指紋と一致する」
    ときだけ。一致しないケース (別の書き手の commit) / 基点を一意に決められない
    ケース (merge) / 予算切れは、すべて空文字 = 復元しない側に倒れること。
    """
    ...
    def restore(self, rel: str, digest: str | None):
        return self.entry._restore_committed_diff(self.repo, rel, digest, self.deadline)

    def digest_of(self, rel: str) -> str | None:
        return self.entry.content_digest(os.path.join(self.repo, rel))

    def commit(self, message: str) -> None:
        _testutil.git(self.repo, "add", "-A")
        _testutil.git(self.repo, "commit", "-qm", message)

    def test_modification_committed_yields_that_commits_diff(self):
        write(self.repo, "seed.txt", "alpha\nBETA\ngamma\n")
        digest = self.digest_of("seed.txt")
        self.commit("edit seed")
        diff = self.restore("seed.txt", digest)
        self.assertIn("-beta", diff)
        self.assertIn("+BETA", diff)
        self.assertNotIn("alpha\n+", diff, "その commit の変更以外を含めない")

    def test_addition_committed_yields_full_file_diff(self):
        write(self.repo, "fresh.txt", "new file\n")
        digest = self.digest_of("fresh.txt")
        self.commit("add fresh")
        diff = self.restore("fresh.txt", digest)
        self.assertIn("+new file", diff)

    def test_deletion_committed_yields_deletion_diff(self):
        os.remove(os.path.join(self.repo, "seed.txt"))
        digest = self.digest_of("seed.txt")
        self.assertEqual(digest, self.entry.DIGEST_ABSENT)
        self.commit("remove seed")
        diff = self.restore("seed.txt", digest)
        self.assertIn("-alpha", diff)

    def test_content_written_by_another_actor_is_not_restored(self):
        """指紋を記録した後に別の書き手が同じパスへ別内容を commit した場合は
        復元しない (これが 0.9.0 の退行の本体)。"""
        write(self.repo, "seed.txt", "alpha\nMINE\ngamma\n")
        digest = self.digest_of("seed.txt")
        write(self.repo, "seed.txt", "alpha\nSOMEONE ELSE\ngamma\n")
        self.commit("another writer")
        self.assertEqual(self.restore("seed.txt", digest), "")

    def test_missing_digest_is_not_restored(self):
        write(self.repo, "seed.txt", "alpha\nBETA\ngamma\n")
        self.commit("edit seed")
        self.assertEqual(self.restore("seed.txt", None), "")
        self.assertEqual(self.restore("seed.txt", ""), "")

    def test_never_committed_path_is_not_restored(self):
        """一度も commit されていない (実体の無い pending エントリ) は基点が無い。"""
        write(self.repo, "phantom.txt", "tmp\n")
        digest = self.digest_of("phantom.txt")
        os.remove(os.path.join(self.repo, "phantom.txt"))
        self.assertEqual(self.restore("phantom.txt", digest), "")

    def test_merge_deciding_the_path_is_not_restored(self):
        """衝突解決を伴う merge がそのパスの内容を決めている場合、書き換え後の親が
        2 つになる。どちらを基点にしてももう片方の変更を巻き込むので復元しない。"""
        ...
        digest = self.digest_of("seed.txt")
        self.commit("merged")
        parents = self.gitscan_parents("seed.txt")
        self.assertEqual(len(parents), 2, "merge commit の親が 2 つになっていない")
        self.assertEqual(self.restore("seed.txt", digest), "")

    def gitscan_parents(self, rel: str) -> list[str]:
        import gitscan

        found = gitscan.last_commit_touching(self.repo, rel)
        self.assertIsNotNone(found)
        return found[1]

    def test_exhausted_budget_skips_restore(self):
        write(self.repo, "seed.txt", "alpha\nBETA\ngamma\n")
        digest = self.digest_of("seed.txt")
        self.commit("edit seed")
        self.deadline = time.monotonic()  # 残り予算ゼロ
        self.assertEqual(self.restore("seed.txt", digest), "")
