...
また基点が古い SHA になる分 diff の対象範囲が `HEAD` 基準より広がりがちで、
`MAX_FILE_DIFF_BYTES` (32KB) の切り詰め (先頭のみ送信・全文 hash で
reviewed 化) が発生する頻度も上がりうる。仕組み自体は 0.5.0 からの
既存の安全弁で新規リスクではないが、発火頻度は 0.7.0 で増える。
