...
検出の堅牢化)。挙動変更を含むため minor bump。**

### explore-parallel: 同時起動数と注入バイト数に上限を入れた

Claude は **1 メッセージで複数の Explore を並列起動する**のが通常で、0.10.0 までは
`pre` が `tool_use_id` ごとに無条件で cursor を起動していた。並列起動された本数だけ
cursor agent が同時に走り (CPU・Cursor の利用量)、`additionalContext` への注入も
本数 × 8000 バイトだけ積まれる。post-implementation-review 側の cursor 直列化
(cwd 単位の flock) に相当する制限がこの hook には無かった。

| 変数 | 既定 | 効果 |
|---|---|---|
| `EXTERNAL_AI_EXPLORE_MAX_CONCURRENT` | `2` | 同時に走らせるアナライザの上限 |
| `EXTERNAL_AI_EXPLORE_MAX_RESULT_BYTES` | `8000` | 1 アナライザの結果として注入するバイト数の上限 |

- **「同時に走る本数 × 1 結果あたりの上限」が 1 度に注入されうる量の頭打ち**になる
  (既定で ≒ 16KB)。結果を持てるのは起動できたアナライザだけなので、同時に生きている
  本数がそのまま注入量の上限として効く。**1 ターンの合計の上限ではない**: アナライザが
  1 本終わると枠が空くので、同じターン中に起動と完了を繰り返せば完了した本数分だけ
  積まれる。注入側に「ターン単位の合計バイト数」の台帳を持たせる案は採らなかった —
  hook payload に「ターン」を一意に表す ID が無く時間窓で近似するしかないうえ、既に
  走り終えた (課金済みの) 結果を捨てる形になるため。上限は使う前に効かせる
- **上限はセッション単位ではなくユーザー・マシン単位**。数え上げは
  `$TMPDIR/explore-parallel/` 全体を見るので、`$TMPDIR` を共有する同一ユーザーの全
  セッションで枠が 1 つ (並列セッションでは先に取ったほうが他を飢餓させる)。CPU /
  利用量を抑える狙いからはマシン単位が正しいのでそのままにし、挙動として文書化した
- **上限で落としたときは `systemMessage` を 1 行出す**。stderr は exit 0 の hook では
  debug log にしか出ないため、それだけだと利用者が「補助調査が付かなかった」ことを
  観測できない (ロック競合は一時的な事象なので従来どおり stderr だけ)
- **注入を上限で切り詰めたら末尾に 1 行記す** (`(結果はここで切り詰めた: 上限 N バイト /
  <env 名>)`)。切るのは末尾・残すのは先頭なので、印が無いと親 Claude は「補助調査は
  ここで終わった」と読む
- 数え方は `$TMPDIR/explore-parallel/` の pid ファイルのうちプロセスが生きているもの
  (`os.kill(pid, 0)` のみ。`ps` は起動しない — pre の hook timeout は 5 秒で GC も
  同じ呼び出しで回るため)。zombie と権限の無いプロセスは「生きている」側に数える
  = 枠を余分に塞ぐ = 起動しない側に倒れる
- **数え上げから起動までを flock で直列化する** (`state.launch_gate()`)。PreToolUse
  hook 自体が同時に複数走るので、排他にしないと双方が「まだ枠がある」と読んで上限を
  超える。ロックを取れなかった回は起動しない (超えない側)。ロックファイルを作れない
  環境では直列化を諦めて進む (fail-open)
- **上限超過分はキューに積まない** (起動せず、通知と stderr の記録だけ)。遅れて届く
  補助調査には価値が無く、キューを持つと「いつ起動されるか分からない cursor」が増える
...
- **CLI の検出はロックを取る前に済ませ、pre の予算を GC と共有する**。0.11.0 の検出は
  キャッシュが無い / 使えない環境で最大 3 秒の probe になったため、`launch_gate()` の
  内側に置くと GC (2 秒) + 検出 (3 秒) + ロック待ち (0.5 秒) で pre の hook timeout
  (5 秒) を超え、hook 自体が kill されて現在のアナライザを起動できない (0.10.0 で GC に
  ついて直したのと同型の失敗)。cold cache では先頭の 1 本が probe する間に他の pre が
  待ちきれず、上限に届かない副作用もあった。`__main__.PRE_BUDGET_SEC` (4 秒) の締切を
  GC の後に残った分だけ検出へ渡し、ロックが保持するのは数え上げと起動だけに戻した

テスト: `tests/test_concurrency_limit.py` (20 件) と `tests/test_posix_guard.py` (3 件)。
注入量は「5 本を並列に pre → 起動 2 本・注入 2 本分」と「逐次に 5 本 pre→post → 5 本分
積まれる」の両方を実測で固定した (掛け算の恒等式では上限が効いているかを判定できない)。
実装行を壊す mutation 10 通り (検出位置 / pre の予算 / 通知の有無と対象 / 切詰の向きと
マーカー / 上限と数え上げ) で先に落ちることを確認した。
...
- **検出順は `cursor-agent` → `cursor`**。曖昧さの無い名前から先に見る。
  `cursor` を使う場合だけ `cursor agent ...` の形で起動する (`subcommand_for`)
- **汎用名 `agent` は候補にしない**。プロダクト名を持たないので、Cursor と無関係な実体
  (社内スクリプト・別ツールの別名) が PATH に居るだけで採用され、3 hook が渡す
  **リポジトリの差分や実装プランがそこへ届く** (起動 argv は `ps` からも見える)。
  候補を 1 つ増やすことは送信先を 1 つ増やすことなので、下振れに上限が無い側は取らない。
  `agent` に本体を置いている環境は `EXTERNAL_AI_CURSOR_COMMAND=agent` で明示指定する
  (probe も飛ぶので従来どおり動く)
- 候補は `--version` で応答するかを確認するが、**採用は候補順が決める**。失格でない
  最初の候補を使い、応答確認 (`PROBE_OK`) は「以降の候補を probe せずに確定できる」
  最適化としてのみ使う。応答の速さで順序をひっくり返すと、cold start の遅い本物
  (`cursor-agent`) が保留になった隙に後ろの `cursor` (IDE ランチャーでありうる) が
  勝ってしまい、この検出が解こうとしていた構成でそのまま失敗する。**応答を確認できない
  (timeout / 予算切れ) 候補は失格にしない** — 遅い本物を切ってレビュー機能が黙って
  止まるほうが、0.10.0 までの「掴んでから失敗を待つ」より悪い。明示的に失敗する候補
  (非 0 終了 / 出力なし) だけを失格にする
- **名前で同定できない候補 (`cursor`) は、`--version` の出力に `cursor` の語がある場合
  だけ「確認できた」とみなす**。無い場合も採用はする (挙動は 0.10.0 と同じ) が、
  キャッシュ TTL を短い側に置く
- 検出結果は `$TMPDIR/external-ai-assist/cursorcli.json` に TTL 付きでキャッシュする
  (**応答を確認できた場合 1 時間 / 見つからない場合と未確認のまま採用した場合 5 分**)。
  `is_available()` は編集ツールのたびに呼ばれるので毎回 probe しない。記録した実体の
  パスが `which` の結果と食い違ったとき、および**同じパスのまま inode / mtime が変わった
  とき** (別のツールを入れ直した・インストーラが置き換えた) は捨てて測り直す。
  ファイルは 0o600
- **共有 `$TMPDIR` に置かれたキャッシュを信用しない**。置き場 (hook が所有する 1 階層) は
  `flock.ensure_private_root` で所有者 / group·other 書込権 / symlink を検査し、
  さらに**記録された command が検出候補 (`CANDIDATES`) に無ければ捨てる**。
  `shutil.which` はセパレータを含む値をそのパスとして解決するため、この 2 つが無いと
  「他ユーザーが先回りしてキャッシュを置き、外部 AI CLI として起動する実体を選ぶ」
  経路ができる (state 系ディレクトリと同じ脅威モデル)
- probe の予算は 1 回 1.5 秒 / 合計 3 秒で、**呼び出し側から `deadline` でさらに縮め
  られる** (explore-parallel の pre は残骸 GC と同じ 5 秒の枠で回るので、GC が使った
  時間を引いた残りを渡す)。予算切れは保留 (`PROBE_UNKNOWN`) に落ちるだけなので、
  機能が黙って止まる方向には倒れない。応答しない probe の後始末は
...
- **キャッシュを使えない環境では hook 1 回ごとに検出が走る** (置き場の所有者 / 権限
  検査に落ちる場合。他ユーザー所有のディレクトリでは chmod での自己修復もできないため
  恒久的にこの状態)。予算内に収まるので機能は止まらないが、probe を完全に省きたい
  環境は `EXTERNAL_AI_CURSOR_COMMAND` で実体を固定する (README の「前提」節に記載)
- post-implementation-review の **pre-tool / post-tool は検出に 2 秒の締切を渡す**
  (`PER_TOOL_PROBE_BUDGET_SEC`)。この 2 フェーズは編集ツールのたびに走り、10 秒の hook
  timeout を git (最悪 7 秒) と分け合うため、検出の既定予算 3 秒をそのまま使うと合計が
  timeout と同着になる (ハーネスの kill が自前の fail-open より先に来る)。
  `tests/test_review_set.py::TestTimeoutBudgets` の突合に検出のぶんを足した。Stop
  (690 秒の枠) は締切なし
...
**未実装 (実機確認が前提)**: IDE ランチャーと Agent CLI を**出力から確実に**見分けること、
および**未ログイン状態の検出**。`--version` はどちらの実体でも 0 で応答し、ログイン状態にも
依存しない (`cursor` の出力に `cursor` の語を求める判定は「確認できた / できない」の
区別に留め、採用の可否には使っていない)。`--help` の内容や失敗文言でのパターン判定は、
実機の出力を確認せずに入れると正常な応答を誤検出してレビュー機能を黙って止めうるので
採らなかった (検出順と候補の絞り込みで「両方が入っている環境で IDE ランチャーを掴む」
ケースは解消する)。このリリースでは実機 (cursor のログイン状態を含む) での確認は
行っていない。

テスト: `hooks/_common/tests/test_cursorcli.py` (検出順と候補順の優先 / 候補の絞り込みと
無関係な実体を起動しないこと / 曖昧な名前の同定と TTL の出し分け / キャッシュの無効化・
置き場の検査・候補名の制限・実体の入れ替わり / 予算と `deadline` / 固定指定 / argv 契約)。
実装行を壊す mutation 16 通り (候補の絞り込み / 同定検査 / 候補順 / 予算と `deadline` /
キャッシュの指紋と TTL / 固定指定 / サブコマンド) で先に落ちることを確認した。
**既知の空振りが 1 件**残っている: probe の kill 猶予 (`PROBE_KILL_GRACE_SEC`) を既定に
戻す mutation は、フィクスチャが TERM で死ぬか無視するかで経過時間が変わるため決定論的に
検出できない (TERM→KILL の段自体は `test_subproc.py` が押さえている)。
「予算が尽きたら probe しない」は経過時間ではなく**起動そのものの回数**で測る —
probe 記録の有無では「起動されたが kill が先に来た」形と区別が付かない。
3 hook のテストは `EXTERNAL_AI_CURSOR_COMMAND` で実体を偽 CLI に固定する
(固定しないと検出が開発機の本物の `cursor-agent` を掴んで `--version` を起動しうる)。

### 見送った変更: 同一ターン内 commit の差分復元 (床テストだけ追加)

pending に積んだ後 Stop までの間にそのパスを commit すると `git diff HEAD -- <path>` が
空になり、その変更はレビューされない (0.10.0 で「取得できませんでした」と通知する
ところまでは入っている)。「編集直後のディスク内容の指紋 (sha256) が Stop 時点の内容と
一致すれば、HEAD に入っているのは自分が書いた内容だ」と証明して復元する案を実装した
が、マージ前レビューで**内容を変えなかった編集**で破れることが分かったため取り消した。

HEAD 基準の diff が空になる最も多い原因は同一ターン内 commit ではなく「編集したが内容
が変わらなかった」(同じ内容の Write、revert して戻した編集) で、このとき指紋は当然
一致する。一方その状況で「そのパスを最後に変更した commit」はこのターンの成果ではなく、
その差分を復元すると**このセッションが一度も見ていない内容** (その commit が消した行)
まで外部 AI CLI へ送ることになる。**送信範囲が広がる側には倒さない**方針に従い、
0.11.0 では復元を出荷せず 0.10.0 の通知のみを維持する。

この失敗を固定する床テストだけ追加した
(`tests/test_stop_flow.py::TestSameTurnCommitNotification`
`::test_noop_edit_does_not_send_an_unrelated_historical_diff`。取り消した実装の上では
落ちることを確認済み)。復元するには「内容がこのセッションのものか」ではなく
**「その commit をこのターンに作ったか」**を示す必要がある (設計は未確定)。
...
レビューで見つかった 1 件は実装のバグ修正を伴う。patch bump)。**
...
**マージ前レビューの指摘 (追加修正)**: explore-parallel の post フェーズは
`is_available()` が False (CLI が PATH から消えた等) だと `analyzer.post()` 自体を
呼ばずに skip していた。pre で既に analyzer を起動済みの場合、この skip は pid
ファイルと稼働中プロセスを孤児にしてしまう (`__main__.py` の「post は起動済み
analyzer を必ず reap する」という不変条件に違反していた)。`is_available()` は
pre (起動可否) のゲートに限定し、post では常に `analyzer.post()` を呼んで reap
するよう修正した。`tests/test_early_returns.py` を「pre 成功 (偽 cursor あり) →
cursor が PATH から消える → post」という遷移で検証するよう書き換え、pid / 結果
ファイルが残らないことを確認する。
