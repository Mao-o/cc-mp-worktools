...
**内部バックログの精査分 3 件 (同一ターン内 commit の差分復元 / Explore 並走の同時起動数と
注入量の上限 / cursor CLI 検出の堅牢化)。挙動変更を含むため minor bump。**

### post-implementation-review: 同一ターン内で commit した変更も (証明できたときだけ) レビューする

pending に積んだ後 Stop までの間にそのパスを commit すると `git diff HEAD -- <path>` が
空になる。0.8.0 以前はこれが黙って消費されて**一度もレビューされずに消えて**おり、
0.10.0 は「差分が空で取得できませんでした」と通知するところまでで、レビュー自体は
行われていなかった。

0.11.0 は **帰属を「内容」で証明できたときだけ復元する**:

| 段階 | どこで | 何をするか |
|---|---|---|
| 記録 | PostToolUse | 編集直後のディスク内容の指紋 (`sha256:<hex>`、存在しなければ `absent`) を state の `digests` に書く。**git は呼ばない** (PostToolUse を軽く保つ設計を維持)。**内容そのものは保存しない** |
| 照合 | Stop | Stop 時点のディスク内容の指紋が一致するときだけ復元する |
| 基点 | Stop | そのパスを最後に変更した commit の (history simplification で書き換えられた) 親 |
| 通知 | Stop | 復元したパスは `systemMessage` に「commit の差分をレビューに使用」、できなかったパスは従来どおり「差分が空で取得できませんでした」 |

**指紋一致がなぜ帰属の証明になるか**: この分岐に入る条件は「HEAD 基準 diff が空」=
ディスクと HEAD の内容が git から見て同一。そこで指紋 (編集直後に自分が書いた内容) と
ディスク内容が一致するなら、**HEAD に入っている内容は自分が書いた内容そのもの**になる。
別のローカルの書き手 (別セッション・人間の手動 commit) が同じパスへ別内容を commit した
場合は一致しないので復元しない (自分の commit の**後**に上書き commit された場合も同じ)。

基点を「pending 記録時点の HEAD」まで遡らせる 0.9.0 の形は、その間にそのパスを**通過した
全変更** (pull で入った他人の commit を含む) を拾って外部 AI CLI へ送るため 0.10.0 で
撤去してある。0.11.0 の基点は「最後にそのパスを変更した commit の親」なので、**送る差分は
commit が挟まらなかった場合と同じ範囲**で、それ以上遡らない。

- 基点の親が 2 つ以上 (衝突解決を伴う merge がそのパスの内容を決めている) なら復元しない
  — どちらの親を基点にしてももう片方の変更を巻き込むため
- 親が 0 個 (そのパスの履歴がその commit で始まる = 純粋な追加) は `/dev/null` 比較
  (untracked 新規ファイルと同じ形) にする。`git rev-list --parents -1 HEAD -- <path>` の
  親が書き換えられることは使い捨て repo 5 形で実測した (親 1 / 親 0 / merge 親 2 /
  pull が混ざった場合 / 削除)
- **追跡ファイルの削除が同一ターン内で commit されたケースも復元できる** (指紋は
  「ディスクに無かった」ことも表せる)。一度も commit されていない一時ファイル
  (実体の無い pending エントリ) は基点が無いので復元されず、通知だけが残る
- **失敗方向は不変: 送信範囲が広がる側には倒さない。** 指紋が無い (0.10.0 以前の
  state ファイル) / 一致しない / 基点を一意に決められない / 収集予算が足りない、は
  すべて「復元しない」に倒れる
- 予算: 指紋は 1 ファイル 1MB (`SNAPSHOT_MAX_FILE_BYTES`) / 1 呼び出し合計 8MB
  (`SNAPSHOT_BUDGET_BYTES`) まで。超えたパスは指紋なし = 復元しない。復元は git を
  2 回呼ぶので、収集予算の残りが 2 回ぶん無ければ試みない (`RESTORE_RESERVE_SEC`)
  — 収集ループ全体の最悪ケースは従来どおり
- 指紋はレビュー完了時に捨てる (`complete_claim`)。`digests` の保持は 200 件で、
  溢れたら古い方から捨てる (失っても「復元しない」に倒れるだけ)

回帰テスト: `tests/test_stop_flow.py::TestSameTurnCommitRestore` (0.9.0 の脆弱性の
再現 2 件を含む)、単体は `tests/test_review_set.py::TestContentDigest` /
`TestRestoreCommittedDiff`。追加分は実装行を壊す mutation 10 通りで先に落ちることを
確認した (1 通りは早期 return のコスト削減行で、消しても判定が変わらない = 空振りと
確認したうえでコメントに明記)。
...
- **1 ターンの注入合計は「同時起動数 × 1 結果あたりの上限」で頭打ち**になる
  (既定で ≒ 16KB)。結果を持てるのは起動できたアナライザだけなので、起動側の上限が
  そのまま注入量の上限として効く。注入側に別途「ターン単位の合計バイト数」の台帳を
  持たせる案は採らなかった — hook payload に「ターン」を一意に表す ID が無く時間窓で
  近似するしかないうえ、既に走り終えた (課金済みの) 結果を捨てる形になるため。
  上限は使う前に効かせる
...
- **上限超過分はキューに積まない** (起動せず stderr に記録するだけ)。遅れて届く補助調査
  には価値が無く、キューを持つと「いつ起動されるか分からない cursor」が増える
