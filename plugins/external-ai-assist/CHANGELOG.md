...
TTL と無効化・置き場の検査・候補名の制限 / 固定指定 / argv 契約)。実装行を壊す
mutation 13 通りのうち 12 通りで先に落ちることを確認した (残り 1 つは probe の kill 猶予で、
TERM を無視するフィクスチャでも group が TERM で死ぬため経過時間に差が出ず落ちない —
テストの docstring に明記した)。
