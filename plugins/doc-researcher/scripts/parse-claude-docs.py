...
def cmd_search_index(args):
    ...
    if not scored:
        ...
        print()
        print("Tip: try broader keywords or 'search-content' for body matches")
    else:
    ...
    print("      For reliable heading-level access, chain via search-content instead.")
    next_hint("search-content", full_cache, '"<query>"')
...
def cmd_search_content(args):
    ...
    if total_hits == 0:
        ...
        print(f"({total_hits} hits across {docs_matched} pages, showing top {printed_docs})")
    print()
...
def main():
    ...
    p_search_body.add_argument("file", help="Path to llms-full.txt file (auto-fetched if missing)")
    ...
    p_search_body.add_argument("--doc-index", type=int, default=None,
                               help="Restrict search to a single page (default: all)")
