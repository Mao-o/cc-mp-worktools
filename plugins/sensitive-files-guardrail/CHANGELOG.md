...
可視性改善 / ログ衛生) + 外部レビュー R1 指摘の反映 2 件 (項目 1 の予算単位と
項目 7 のローテーション直列化)。**判定境界 (deny / allow / ask / block するか)
の変化: なし。** テスト件数: redact 1185 → **1207**、check 94 → **119**。
...
1. **block reason に出力予算を追加し、ファイル数が多いと際限なく伸びて
   ...
   レシピ) を先に確保し、残り予算をファイル列挙
   (`  - path` 行) に充て、溢れた分は `... (N more files; see git status)` に
   畳む。tail 自体は truncate しない (固定部分だけで予算を超える極端なケース
   では出力が予算を超えることを許容する — 案内を失う方が実害が大きい
   ため)。tracked / untracked の両方に該当があるときは残り予算を半分ずつに
   分ける (先着順だと処理順が先の tracked が予算を独占し、untracked が
