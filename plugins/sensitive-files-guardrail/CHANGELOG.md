...
- `_lex` の算術文脈追跡 (`$((…))` / `$[…]`) が**追跡しない文脈**を docstring と
  MATRIX に明文化: 配列添字 `a[1<<2]=x` / `${a[1<<2]}` / `${#a[1<<2]}` と部分文字列
  `${x:1<<2:2}` では bash は `<<` をシフトとして解釈するが本 hook は heredoc 扱いの
  まま。偶然の delimiter に完全一致する行が後続にあるときだけ間の行が本文扱いで
  消える (無ければ fallback で行分割し deny)。裸の `[` は非クォートで日常的に現れる
  ため、`[` で算術状態に入ると heredoc 検出が広範囲に止まり悪化しうる — 意図的に
  追跡しない (マージ後の事後レビューの指摘)
...
- 予算 (12s) と `hooks.json` の Stop timeout (15s) は別ファイルにあるため、
  「予算が timeout より 1 秒以上小さい」ことをテストで突合する (timeout を下げると
  予算が黙って無意味になり、この修正が直した無音 fail-open に戻るため)
- **トレードオフの開示**: 検査に **12〜15 秒かかっていた repo では、旧版は完走して
  全件報告できていたのに本版は打ち切って一覧が減る**。判定表 (block / pass) は
  変わらないが**報告内容は減る**方向の変化で、mutation テストでは原理的に検出
  できない型 (旧版が拾えていた入力を落とす)。15 秒に到達すれば旧版は報告が
  丸ごと消える (kill + discard) ので「部分報告 + 打ち切りの明示」の方を選んだ。
  打ち切りは必ず表示されるので、黙って減ることはない
...
バックログの精査で発見) + 隔離内レビュー指摘の反映 5 件 + 外部レビュー指摘の
反映 1 件。**判定境界 (deny / allow / ask) の変化: なし。** 変わるのは reason
文字列の正確さ・情報量だけ (Read/Bash/Edit/Write × permission_mode 3 種 ×
fixture 30 個 = 1,623 件のコーパスを 0.25.0 と突き合わせ、
`permissionDecision` の差分 0 / reason の予算超過 0 を確認)。テスト件数:
redact 1119 → **1185** (check 94 は変化なし)。
...
4. **reason の byte 予算 (3KB) を超える折り畳みで `</DATA>` 閉じタグと末尾
   ...
   (note を守ると **明細行 (key 行) が 0 行になる** 入力では、note を諦めて
   予算を key 行に回すフォールバック付き。閉じタグと省略マーカーは常に残す)。
   なお note と省略マーカーの next action が固定費として乗るぶん、**予算が
   元々きつい Edit / Write の minimal info では表示される key 行が 0.25.0
   より減る**ことがある (コーパス実測: 1,623 件中 366 件で 1〜9 行減。減るのは
   Edit / Write のみ)。免責事項と閉じタグを優先した意図的なトレードオフだが、
   **0 行まで落ちる組み合わせは無い** (下記 10 で解消)。同コーパスで 264 件は
   逆に増える。
...
6. **keys-only scan で鍵名が 1 つも表示されなくなる退行を修正**
   ...
   告知を末尾行から header 行 (`keys (in order, max 60 shown):`) へ移した。
   あわせて省略マーカーの単位を keys-only のブロックだけ `lines` → `keys` に
   し、「鍵名を何個落としたか」が読めるようにした (件数の数え方は下記 11)。
...
9. **空の dotenv で末尾 note が出ない不具合を修正**。`format_dotenv` の
   ...
   出す (上記 4 と合わせ、`<DATA>` ブロック内に per-format の `note:` 行が
   無いケースはコーパス 1,623 件中 0.25.0 の 627 件 → 120 件。残る 120 件は
   下記 10 のフォールバックが発火して note より key 行を優先したものだけ)。
   なお **`<DATA>` 包装の header にある `NOTE: ... Real values are NOT in
   context.` は常に残る** — `_fit_data_block` は header 3 行が入らない限り
   何も返さないため。フォールバックが手放すのは per-format の補足説明
   (「型・長さ・status タグしか返していない」等) であって、実値が context に
   無いという開示そのものではない (実測: 発火した 120 件すべてで header の
   `NOTE:` 行と `format:` 行が残存)。
10. **note 保護のフォールバックが発火せず、Edit / Write の minimal info が
    「見出しと省略マーカーだけ」になる退行を修正** (上記 4 の初版が持ち込んだ
    もの)。フォールバック条件が「1 行も採用できない」で書かれていたが、
    `<DATA>` ブロックには header と `format:` / `entries:` の見出しが必ず
    入るため空になることはなく、**明細行 (key 行) が 0 行に後退しても発火
    しなかった**。条件を「明細行が 0 になるとき」に改め、そのとき note 保護
    を外して浮いた予算を key 行に回すようにした (閉じタグと省略マーカーは
    どちらの段でも残す)。note 保護を外しても明細行が増えないブロック
    (`(no entries)` 等、明細行を持ちえない形) では従来どおり note を残す。
    コーパス実測: 「0.25.0 では key 行が見えていたのに 0 行」が 114 件 → **0 件**
    (判定差分 0 / 予算超過 0 / 明細行が減った case 0)。
11. **keys-only scan の見出しと省略マーカーが「何個見えていないか」を偽る
    不具合を修正** (上記 6 の初版が持ち込んだもの)。preview 上限 (60) を
    告げる見出しが `first 60 shown` という断定形だったため、そのあと予算で
    **さらに**畳まれても文言が残り、実際には 23 行しか出ていないのに
    「60 個表示」と言っていた。さらに省略マーカーの件数が「落とした**行数**」
    だったので、preview 段階で隠れた分を数え落としていた (500 鍵で
    `... (37 more keys)` = 477 件不可視を隠蔽)。見出しを上限表現
    (`max 60 shown`) にし、件数を **`entries:` の総数 − 実際に残った鍵行数**
    に改めた。`entries:` を読めない・総数と噛み合わないブロックは従来どおり
    行数ベース。コーパス実測: 見出しが実際より多く見せていた 84 件 → **0 件**、
    「表示鍵数 + マーカー件数 = entries」の会計が合わない 84 件 → **0 件**
    (判定差分 0 / 予算超過 0 / 明細行が減った case 0 / 3 件は逆に増加)。
    なお件数は `keys-only scan` が抽出する上限 (500 鍵) を母数とする —
    それを超える鍵を持つファイルでは `entries:` 自体が頭打ちになる。
...
(`sensitive-files-guard` → `sensitive-files-guardrail`, 0.14.0 直後の
commit 52113a1) で custom `patterns.local.txt` の参照ディレクトリが
`~/.claude/sensitive-files-guard/` → `~/.claude/sensitive-files-guardrail/` に
変わった。旧ディレクトリに custom rule を残したまま upgrade した既存ユーザは、
`load_patterns()` が新パスしか読まないため、rename だけで黙って rule を失って
いた (include rule 消失 = プロジェクト固有の機密が露出 / exclude rule 消失 =
以前 allow したものが再び block)。進行中の 1.0.0 ロードマップ (Unreleased)
...
- **`hooks/_shared/patterns.py`**: `load_patterns()` に rename 前の旧パス
  `~/.claude/sensitive-files-guard/patterns.local.txt` への fallback 読み込みを
  追加。
  - 新パス `~/.claude/sensitive-files-guardrail/patterns.local.txt` が存在すれば
    **それのみ**を読む (従来挙動・旧パスは見ない)。
  - 新パスが無く旧パスがある場合のみ、旧パスを fallback で読み込み保護挙動を
    維持する + 移行警告を出す。
  - **両方存在する場合は新パスを優先し旧パスは無視する** (マージしない)。
    移行済みユーザの現行設定を権威とするため。last-match-wins セマンティクス上、
    旧パスを後ろに連結すると古い rule が勝って現行意図を上書きしてしまうので、
    安全側として旧パスを完全に無視する判断。
  - 既存契約は不変: 既定 patterns.txt → ローカルの順 last-match-wins、
    `FileNotFoundError` は黙殺、旧パス読込でも FileNotFound 以外の OSError は
    従来同様 `warn_callback` に委譲。
  - 移行警告は新設の任意引数 `migrate_warn_callback` 経由 (既存呼出は無改修で
    動く後方互換追加)。callback に渡すのは固定トークン
    `legacy_patterns_local_in_use` のみ (パス・値を含めない —
    `core.logging` の detail 文字種ホワイトリストを通す + ログ秘密非混入の鉄則)。
- **`hooks/check-sensitive-files/checker.py`** (Stop 側): `migrate_warn_callback`
  を注入。旧パス検出時に stderr へ移行案内 (旧 → 新パスへ `mv` を促す) を 1 行
  出す。
- **`hooks/redact-sensitive-reads/core/patterns.py`** (Read 側):
  `migrate_warn_callback` を注入。`core.logging.log_error` で category
  `local_patterns_legacy_path` として logfile + stderr に記録する。
- **注意**: この旧パスは plugin **rename** 由来で、docstring が言及する 0.6.0 で
  削除した XDG fallback (`$XDG_CONFIG_HOME` / `~/.config`) とは別物。XDG
  fallback は復活させない。

### 動作変化

- 旧 `~/.claude/sensitive-files-guard/patterns.local.txt` に custom rule を持つ
  既存ユーザが、新パスに未移行でも従来どおり保護挙動を維持できるようになった
  (silent data-loss の解消)。
...
- 累計 **711 件** (redact 684 / check 27)。redact 側に +10
  (`tests/test_patterns_loader.py`)。
- 新規: `TestCorePatternsLoader` に旧パス fallback / 新パス優先 / 両方不在の
  3 ケース、`TestSharedLegacyFallback` ×5 (旧パス解決 / migrate callback 固定
  トークン発火 / トークンの log-safe 性 / 新パス優先で migrate 無発火 / 旧パス
  OSError は warn_callback に委譲) 、`TestCheckerLoaderContract` に Stop 側の
...
character-level parser、RedirectForm タグ、`<GUARDRAIL_DENY>` 構造化包装を削除。
...
2. **A2: `RedirectForm` タグ (M5, 0.5.0) を撤廃**
   ...
   `form` キーワード引数、GUARDRAIL_DENY body の `form: <値>` 行を削除。
   A1 と一体で input redirect 系の deny 経路自体を撤廃したため、form 情報を
   返す価値が消滅した。

3. **A3: `<GUARDRAIL_DENY>` 構造化包装 (M4, 0.4.2) を plain text に戻す**
   — `core/messages.py` から `_wrap_guardrail_deny` / `_GUARDRAIL_GUARD` /
...
| Bash deny の reason 文字列 | `<GUARDRAIL_DENY tool reason guard>...</GUARDRAIL_DENY>` | plain text (note / matched_operand / first_token / suggestion を改行区切り) |
| Edit/Write deny の reason 文字列 | `<GUARDRAIL_DENY tool="..." reason="sensitive_path[_symlink/_special]">` | plain text + extra_note 行で symlink/special 文脈を表現 |
| `policy_unavailable("deny")` の reason | `<GUARDRAIL_DENY tool="Hook" reason="policy_unavailable">` | plain text (1 段落) |
...
- `core.messages.edit_deny`: `kind` キーワード引数を削除。symlink / special
  ...
  渡していた箇所を撤去済み。
- `core.messages` から削除: `_wrap_guardrail_deny`, `_GUARDRAIL_GUARD`, `BashDenyKind`,
...
判定 / process sub word boundary 等のコード負債と、GUARDRAIL_DENY schema docstring
が想定より嵩んでいたため)
...
3. **A7: 2-tier lookup の fallback 削除** — `patterns.local.txt` の参照先を
   `~/.claude/sensitive-files-guardrail/patterns.local.txt` 単一パスに縮小。
   0.4.0〜0.5.x で fallback として参照していた
   `$XDG_CONFIG_HOME/sensitive-files-guardrail/patterns.local.txt` /
   `~/.config/sensitive-files-guardrail/patterns.local.txt` を撤去
...
| `~/.config/sensitive-files-guardrail/patterns.local.txt` のみ存在 | fallback 採用 + deprecation 通知 | 無視 (preferred 不在として扱う) |
...
- **旧パスからの移行が必要**: `~/.config/sensitive-files-guardrail/patterns.local.txt`
  または `$XDG_CONFIG_HOME/sensitive-files-guardrail/patterns.local.txt` を使って
  いた利用者は手動で
  `mv "${XDG_CONFIG_HOME:-$HOME/.config}/sensitive-files-guardrail/patterns.local.txt" ~/.claude/sensitive-files-guardrail/patterns.local.txt`
...
GUARDRAIL_DENY 撤去), **A4 / B2 / B3** (PR 3), **E1〜E6** (PR 4-6: 思想 2 の意図
汲み取り強化)。
...
/ ``quoted``) を付与して GUARDRAIL_DENY body に出力する。これにより後段 hook /
...
4. **`bash_deny` に `form` キーワード引数追加** (`core/messages.py`):
   ...
   GUARDRAIL_DENY body の ``matched_operand:`` / ``first_token:`` の後、``suggestion:``
...
### `<GUARDRAIL_DENY>` schema 拡張

```
<GUARDRAIL_DENY tool="Bash" reason="input_redirect" guard="guardrail-v1">
note: ...
matched_operand: .env
...
- **新設** `tests/test_bash_handler.py::TestInputRedirectFormInReason`
  — 8 件。handle() 経由で GUARDRAIL_DENY body に ``form:`` 行が出る regression
...
- GUARDRAIL_DENY body に新規行 ``form: <値>`` 追加 (input_redirect 系のみ)。
...
**deny 系 reason の `<GUARDRAIL_DENY>` 構造化包装**。LLM が deny の根拠を機械的に
...
1. **`<GUARDRAIL_DENY tool reason guard>` 構造化包装** — 全 deny reason を以下の
   schema で包む:

   ```
   <GUARDRAIL_DENY tool="<Bash|Edit|Write|MultiEdit|Hook>" reason="<kind>" guard="guardrail-v1">
   note: <人間向け説明文>
...
4. **`escape_xml_tag(text, tag_name)` 一般化** —
   ...
   (後方互換)。Read 側 `<DATA>` 包装と Bash/Edit 側 `<GUARDRAIL_DENY>` 包装が
   同じ防御層を共有する。

5. **テスト追加** — 575 + 27 = 602 件:
   - `tests/test_messages.py::TestSfgDenyEnvelope` 12 件 — 外殻属性、reason 値、
     body 各行、外殻破壊耐性 (body に偽 `</GUARDRAIL_DENY>` を仕込んでも 1 組のみ)、
...
grep -E '^<GUARDRAIL_DENY tool="Bash" reason="(literal|glob|input_redirect)' deny.log

# Edit/Write の symlink 経由 deny だけ抽出
grep '^<GUARDRAIL_DENY tool="(Edit|Write|MultiEdit)" reason="sensitive_path_symlink"' deny.log
```
...
M4 (`<GUARDRAIL_DENY>` 構造化包装) / M5 (リダイレクト形式タグ) / L1〜L5 / B
(bashlex 採否) は次リリース以降。
...
`$XDG_CONFIG_HOME/sensitive-files-guardrail/patterns.local.txt`
(未設定時 `~/.config/sensitive-files-guardrail/patterns.local.txt`) のみを参照して
いたが、Claude Code のユーザー設定と同じディレクトリツリーに集約するため
`~/.claude/sensitive-files-guardrail/patterns.local.txt` を優先パスとして追加。
...
既存利用者は `~/.claude/sensitive-files-guardrail/patterns.local.txt` へ移行する
...
8. **DATA タグ強化** — 固定 `guard="guardrail-v1"` marker、body の `</DATA>` /
