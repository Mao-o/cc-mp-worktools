...
各テストファイルは先頭で ``from _testutil import FIXTURES`` (または
副作用目的の ``import _testutil``) する慣例だが、``_testutil`` は
``tests/`` 直下にある単なるモジュールで、bare import で解決するには
``tests/`` ディレクトリ自体が sys.path に載っている必要がある。
``unittest discover tests`` はディレクトリ探索の top-level directory として
``tests/`` 自体を検索パスに足すため問題にならないが、
``python -m unittest tests.test_matcher`` のようにモジュールパスを直接
...
``redact-sensitive-reads/`` / ``hooks/`` (``_testutil.py`` が追加で挿入する
のと同じパス) を sys.path に載せる。SFG_LOG_PATH の bootstrap は
``_testutil.py`` / conftest.py 側の責務のまま変更しない
(このファイルは sys.path 整備のみ)。
"""
