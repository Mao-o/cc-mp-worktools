...
def _lex(command: str) -> list[tuple[str, str]]:
    ...
    - **算術式の中の ``<<`` はシフト演算子** (Codex R1 / R2 P1): クォート外の
      ...
      **追跡しない算術文脈** (既知の限界、0.33.2 で明文化): 配列添字
      ``a[1<<2]=x`` / ``${a[1<<2]}`` / ``${#a[1<<2]}`` と部分文字列 ``${x:1<<2:2}``
      では bash は ``<<`` をシフトとして解釈するが、ここでは heredoc 扱いのまま。
      偶然の delimiter (``2]=x`` / ``2]}`` / ``2:2}``) に完全一致する行が後続に
      あるときだけ間の行が本文扱いで消える。裸の ``[`` は非クォートで日常的に
      現れる (``a[0]`` / ``[ -f x ]`` / glob ``[a-z]``) ため、``[`` で算術状態に
      入ると heredoc 検出が広範囲に止まり悪化しうるので意図的に追跡しない
    """
    out: list[tuple[str, str]] = []
    n = len(command)
    i = 0
