...
- **plugin rename の最終判断**: 思想 (「うっかり予防のついでに少し守れれば
  ...
  - **2026-06-14 追記**: 検討は **rename 完了済み** で確定 (commit 52113a1,
    `refactor(sensitive-files-guardrail)!: rename from sensitive-files-guard`)。
    元々 plugin 公開名は最初から `sensitive-files-guardrail` だったため
    breaking change ではなく、worktools 内部呼称の表記揺れ
...
E5 (json/toml/yaml status 拡張) はコード/テストが既に commit `3189d907`
(2026-05-24, 0.14.0 リリースより前) で取り込み済み。本セッションで残作業
(ドキュメント反映 + roadmap 表更新) を実施し、Unreleased section として
CHANGELOG.md に集約。E6 / D1 / D2 を続けて取り込み次第 1.0.0 として cut する。

#### 実装済み (commit 3189d907)

- **E5**:
  - `redaction/jsonlike.py::_classify_str_status`: str scalar に `<set>` /
    `<empty>` / `<placeholder>` / `<long>` / `<looks_truncated>` + `length=N`
    + `matched="..."` を付与。`<short>` は対象外 (型クラス前提のため判定不能)。
    bool / num / null / array / object には status を出さない
  - `redaction/tomllike.py::format_toml`: `format_jsonlike(info)` 直流用に
    簡素化 (`_walk` 共有で同 status を自動付与)
  - `redaction/opaque.py::_redact_yaml`: 簡易 top-level key 抽出 + nested
    件数のみカウント。完全パースしない (思想 1)。`_YAML_MAX_TOP_KEYS=500` cap

#### ドキュメント反映 (2026-06-15 セッション)

- `CHANGELOG.md`: `## Unreleased (PR 6, 1.0.0 予定)` セクション新設 + E5 エントリ
- `README.md`: dotenv 例の後に JSON / YAML の minimal info 出力例を追加
- `docs/DESIGN.md`: 「json/toml/yaml の status 拡張 (Unreleased, E5)」サブ
  セクション追加 + dotenv との status タグ整合表
- `docs/MATRIX.md`: Read handler 表脚注に E5 への参照を追加 (format 横断 status)

#### テスト結果

- 累計 **701 件維持** (redact 674 / check 27)。E5 のテスト群 (TestJsonStatus 9 /
  TestTomlStatus 3 / TestYamlExtraction 8 = 計 20 件) は 0.14.0 リリース時点で
  既に 701 件 OK に含まれている (commit 3189d907 が 0.14.0 commit の先行のため)

#### 完了状況サマリ (Unreleased 時点)
...
| P6 | E5 (json/toml/yaml status) | **Unreleased ✓** (commit 3189d907 + docs 2026-06-15) |
| P6 | E6 (Edit/Write リッチ化) | 未着手 (PR 6 残) |
| P6 | D1 / D2 (docs / tests 整理) | 未着手 (PR 6 残) |

### 2026-08-23: docs 整合 (0.19.1) — 出荷版の確定と PR 6 残の再見積り

コード変更なし。2026-08-22 の plugin 精査 (内部バックログ 5 件) で見つかった
docs の不整合を解消した。
...
| P6 | E6 (Edit/Write リッチ化) | **0.20.0 ✓** |
| P6 | D1 (docs 整理) | 未着手・再見積り済み (内部バックログ)。(a) 公開保守者ガイド (docs/MAINTAINING.md) のみ 0.19.1 で先行 |
| P6 | D2 (tests 整理) | **0.33.0 で着手** (下の「2026-09-19」節)。数値目標 (≈500 件) は廃止し「同型ケースの subTest 畳み込み」に再定義 |

### 2026-09-19: D2 着手 (0.33.0) — 数値目標の廃止と畳み込み方針の確定

**決定 1: 当初の数値目標 (≈500 件) を廃止する。**
0.5.0 の計画時 (670 件 → 約 500 件) から実測は 0.14.0 674 → 0.17.0 735 →
0.32.0 1,387 (redact) と一貫して増え続けており、目標値は既に意味を失っていた。
より重要なのは、**件数を目標にすると形状カバレッジを削る側に圧力がかかる**点。
2026-08-28 のレビューで見つかった重大不具合 3 件 (秘密鍵の生バイト漏洩 /
UTF-16 `.env` の「空ファイル」誤報告 / shlex・fnmatch の latency cliff) は
いずれも「件数 900 超でも見えていなかった入力形状の空白」が原因で、件数と品質は
相関していなかった。

**決定 2: D2 は「同型ケースの subTest 畳み込みで格子点を増やす」に再定義する。**
削るのはメソッド数だけで、(入力形 × mode) の格子点は増やす。

0.33.0 の実施分 — `ask_or_allow` に倒れる 6 クラスを
`tests/test_bash_handler.py::AskOrAllowMatrix` 土台へ畳んだ:

| クラス | 畳む前のメソッド数 | 畳んだ後 | 入力形 | 格子点 (形 × 5 mode) |
|---|---|---|---|---|
| `TestHardStopLenient` | 12 | 2 | 6 | 30 |
| `TestInputRedirectAskOrAllow` | 3 | 2 | 2 | 10 |
| `TestShellKeywordLenient` | 8 | 2 | 6 | 30 |
| `TestOpaquePrefixAskOrAllow` | 12 | 2 | 10 | 50 |
| `TestPrefixWithOptionsOpaque` | 8 | 2 | 5 | 25 |
| `TestGlobUncertainAskOrAllow` | 12 | 2 | 7 | 35 |
| **計** | **55** | **12** | **36** | **180** |

畳む前は「コマンド 1 形 × その場で選んだ mode 1〜3 列」で、**形ごとに測る mode が
まちまち**だった (`coproc cat .env` は default だけ、`cat "$X"` は
default/auto/bypass の 3 列)。土台が mode 5 列を必ず回すので、メソッド数が
55 → 12 に減っても検証点は 55 → 180 に増える。

土台を `unittest.TestCase` の非継承にしたのは、空 `CASES` のまま収集されて
「常に通る 2 件」になるのを防ぐため。`CASES` 空のまま使う事故は各メソッド冒頭の
assert で落とす。

**残スコープ**: `test_bash_handler.py` のその他の同型群
(`TestMetadataOnlyAllow` / `TestSafeReadAllowlist` / `TestQuoteAwareHardStop` 等)。
ただし**形状カバレッジの整備を先に済ませること** — 畳む過程で残り少ない形状を
削る危険があるため、順序を逆にしない。
