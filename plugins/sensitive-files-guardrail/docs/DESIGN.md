...
| `move` | `cp` / `mv` | 「コピー / 移動」note + 1Password CLI / pass / git-secret + `.env.example` 派生推奨。basename が `.envrc` family (`*.envrc`、大文字小文字問わず) なら、`.env.example` (Next.js 慣例) ではなく `<元ファイル名>.example` テンプレート派生を案内する専用の**固定文言** (`_BASH_MOVE_SUGGESTION_FORMAT_ENVRC`) に差し替える (0.29.1)。文言は basename に依存せず family 全体で同一 — 0.29.1 の途中版は basename から `<basename>.example` と `cp` コマンド文字列を動的に組み立てていたが、生成物の表面 (quote / glob / operand のディレクトリ保持 / オプション終端) ごとにマージ前レビューの指摘が続いて収束しなかったため撤回した。実ファイル名との対応は同じ reason 内の `matched_operand:` 行から復元できるので、実行可能なコマンド文字列を生成する必要はない。glob operand は既定文言のまま (`bash_handler._build_deny_response` が operand の glob 文字を検知して `is_envrc` を False に固定する。上記 load と同じ修正、マージ前レビューの指摘) |
