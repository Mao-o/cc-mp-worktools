...
| `load` | `source` / `.` | 「shell load」note + minimal info + direnv (.envrc) / dotenv-cli / 1Password CLI 推奨。`.envrc` 自体は direnv hook 経由の読込を案内 (dotenv-cli は出さない、0.29.1)。glob operand (`.envrc*` 等) は対象が確定しないため既定文言のまま |
| `move` | `cp` / `mv` | 「コピー / 移動」note + 1Password CLI / pass / git-secret + .env.example 派生推奨 (`.envrc` は `.envrc.example` 派生、0.29.1。glob operand は既定文言のまま) |
