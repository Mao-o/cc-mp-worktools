...
| `load` | `source` / `.` | 「shell load」note + minimal info + direnv (.envrc) / dotenv-cli / 1Password CLI 推奨。basename が **literal** `.envrc` (大文字小文字も一致、`engine.is_envrc_basename`) のときだけ direnv hook 経由の読込を案内 (dotenv-cli は出さない、0.29.1)。`foo.envrc` のような命名付きスクリプトや大文字小文字違いの `.ENVRC` は direnv が自動発見しないため対象外で既定 (dotenv-cli を含む) 文言のまま (マージ前レビューの指摘、0.29.1)。glob operand (`.envrc*` 等) も対象が確定しないため既定文言のまま |
| `move` | `cp` / `mv` | 「コピー / 移動」note + 1Password CLI / pass / git-secret + .env.example 派生推奨。basename が **literal** `.envrc` のときだけ `.envrc.example` 派生の案内 (0.29.1)。`foo.envrc` / `.ENVRC` は literal ではないため `.env.example` の既定文言のまま (実際の basename と異なる名前を勧めない、マージ前レビューの指摘、0.29.1。glob operand も既定文言のまま) |
