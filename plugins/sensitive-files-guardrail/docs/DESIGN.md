...
- 単位ラベルは既定が `lines`。**keys-only scan のブロックだけ `keys`**
  (`format:` 行の marker で判定)。1 行 = 1 鍵なので「鍵名を何個落としたか」を
  そのまま言える
- 閉じタグの直前が per-format の `note:` 行 (「実値は無い」の免責事項) なら、
  それも固定 tail として保護を試みる。ただし note を守ると **1 行も採用でき
  なくなる** 入力では note を諦めてもう一度畳む 2 段構え。
  note と省略マーカーの next action は固定費なので、**予算が元々きつい
  Edit / Write の minimal info では表示 key 行が 0.25.0 より 1〜2 行減る**
  (鍵数の多い dotenv の上書きでは 0 行になる組み合わせがある)。免責事項と
  閉じタグを優先した意図的なトレードオフで、Read / Bash では逆に増える
