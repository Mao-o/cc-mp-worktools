...
| `load` | `source` / `.` | 「shell load」note + minimal info + direnv (.envrc) / dotenv-cli / 1Password CLI 推奨。basename が `.envrc` **family** (`*.envrc`、大文字小文字問わず、`engine.is_envrc_basename`) なら dotenv-cli を出さず shell script 前提の文面にする (0.29.1)。family の中でも **literal** `.envrc` (`engine.is_direnv_literal`) のときだけ direnv hook 経由の自動読込を案内し、`foo.envrc` / `.ENVRC` のような非 literal な family は「direnv の自動発見対象外」に留めて自動読込は案内しない (マージ前レビューの指摘、2 段階の判定に分離)。glob operand (`.envrc*` 等) は対象が確定しないため既定文言のまま |
| `move` | `cp` / `mv` | 「コピー / 移動」note + 1Password CLI / pass / git-secret + `.env.example` 派生推奨。basename が `.envrc` family (`*.envrc`、大文字小文字問わず) なら、実際の basename から動的に組み立てた `<basename>.example` 派生の案内に差し替える (`.envrc` → `.envrc.example`、`foo.envrc` → `foo.envrc.example`、`.ENVRC` → `.ENVRC.example`、0.29.1。マージ前レビューの指摘でテンプレート導出を basename 派生に変更 — literal かどうかは問わない)。glob operand は既定文言のまま |
