...
- ``open_regular`` は ``O_NOFOLLOW`` で open して ``fstat`` でサイズを取得し
  fd を返す (path の再 open を排除 → うっかり symlink follow を防ぐ)
- ``is_regular_directory`` は Edit/Write handler 向け (Step 6)。最終要素が
  symlink/special でないことだけ保証する (親ディレクトリの差し替え race は範囲外)

0.6.0 で ``open_regular`` の ``fstat`` による S_ISREG 再確認を撤廃した。
``classify`` 済み regular path のみを呼ぶ契約は維持され、敵対的 race
(open 直前の差し替え) は思想 1 の対象外。
...
def open_regular(path: Path) -> tuple[int, int]:
    ...
    classify 済みで ``regular`` のパスでのみ呼ぶこと。``O_NOFOLLOW`` で open
    して symlink follow を防ぎ、``fstat`` でサイズを取得して fd と合わせて返す。
    ...
    Raises:
        OSError: open 失敗、symlink 経由 (UNIX: ``ELOOP``) など
    """
