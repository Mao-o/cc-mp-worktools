...
> 露出につながらない操作だった。`ls` / `find` / `tree` / `stat` / `file` /
> `du` / `df` / `test` / `wc` / `basename` / `dirname` / `realpath` /
> `readlink` / `echo` / `printf`、および `git check-ignore` / `git ls-files` /
> `git status` (subcommand 直書き形) は operand の内容を stdout に出さないため、
> 機密 operand でも **allow** に倒す。`cat` / `head` / `grep` 等の内容出力系と
> `cp` / `mv` (複製で漏洩面が広がる)、`git show` / `git diff` / `git add` は
> 従来通り deny 固定。`echo KEY=val > .env` のような書込み形は residual
> metachar の ask 経路が先に効くため緩まない。
...
- `~/.claude/sensitive-files-guard/patterns.local.txt` (0.6.0 から単一パス)

> 0.4.0〜0.5.x で fallback として参照していた
> `$XDG_CONFIG_HOME/sensitive-files-guard/patterns.local.txt` /
> `~/.config/sensitive-files-guard/patterns.local.txt` は **0.6.0 で撤去**。
> 旧パスを使っていた場合は手動で
> `mv "${XDG_CONFIG_HOME:-$HOME/.config}/sensitive-files-guard/patterns.local.txt" ~/.claude/sensitive-files-guard/patterns.local.txt` する。
...
```bash
# redact-sensitive-reads (646 tests, 0.14.0)
