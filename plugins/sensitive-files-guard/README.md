...
### `PreToolUse(Bash)` — redact-sensitive-reads (0.2.0)

Claude が `Bash` で機密ファイルに触れる単純コマンドを実行しようとすると、
...
| `cat $X`, `cat "$X"`, `cat $(echo .env)` | **ask_or_deny** | 変数展開 (判定不能) |
| `cat .env && pwd`, `cat .env; true`, `cat .env \| head` | **ask_or_deny** | 複合コマンド (判定不能) |
| `< .env cat`, `cat << EOF ... EOF` | **ask_or_deny** | リダイレクト/heredoc (判定不能) |
| `cmd1\ncmd2` (改行区切り) | **ask_or_deny** | 複数コマンド (判定不能) |
| `/bin/cat .env`, `./cat .env`, `../bin/cat .env` | **ask_or_deny** | 絶対/相対パス実行 (判定不能) |
| `FOO=1 cat .env`, `env X=1 cat .env` | **ask_or_deny** | env prefix (判定不能) |
| `bash -c "cat .env"`, `sh -c "..."`, `zsh -c "..."` | **ask_or_deny** | shell wrapper (判定不能) |
| `sudo cat .env`, `command cat .env`, `xargs -a .env cat` | **ask_or_deny** | 権限/ラッパ (判定不能) |
| `python -c "..."`, `node -e "..."`, `ruby -e "..."` | **ask_or_deny** | インタプリタ経由 (判定不能) |
...
**クォート内の metachar も保守的に fail-closed** になります: `echo "hello | world"`
や `git log --format='{"sha":"%H"}'` などは shell の parse 上は安全ですが、hook
側では文字列レベルで `|` / `{` を検出するため ask_or_deny に倒れます。機密 path
を触らない用途なら非 bypass モードで allow 可能、bypass モード下では deny されます。
...
2. **Bash 間接アクセス** — `< .env`, `command cat`, `env VAR=... cat`,
   ...
   `bash -c`, `bash -lc`, `FOO=1 source .env`, 改行区切り複数コマンドなどは
   静的解析不能のため全て **ask (fail-closed)** で倒す
