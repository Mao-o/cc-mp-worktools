...
> `cat` / `head` / `grep` 等の内容出力系と `cp` / `mv` (複製で漏洩面が広がる)、
> `git show` / `git diff` / `git add` は従来通り deny 固定。`echo KEY=val >
> .env` のような書込み形は residual metachar の ask 経路が先に効くため緩まない。
...
```bash
# redact-sensitive-reads (661 tests, 0.14.0)
