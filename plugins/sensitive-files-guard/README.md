...
- **deny 固定**: 機密 path 一致 / glob 候補列挙 hit / 前置き剥がし後の確定 match。
  bypass / auto を含めて全 mode で block
...
呼び出しでも operand が機密パターンに literal 一致すれば deny される。許可したい
場合は `patterns.local.txt` に `!<basename>` を追加する
...
```bash
# redact-sensitive-reads (518 tests, 0.9.0)
