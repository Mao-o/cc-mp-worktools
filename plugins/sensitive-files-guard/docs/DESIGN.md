...
`_METADATA_ONLY_FIRST_TOKENS` (`ls find tree stat file du df test wc basename
...
スキップして allow** に倒す (機密 operand でも deny しない)。
...
- `find -exec cat {} +` は `{}` が hard-stop のため segment 単位で ask 維持。
