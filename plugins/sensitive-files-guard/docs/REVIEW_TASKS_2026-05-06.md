...
`sensitive-files-guard` v0.5.0 の **思想ベース再評価** レビュー結果。
...
- **対象**: `handlers/bash/redirects.py::RedirectForm`,
  ...
  返却部、`core/messages.py::bash_deny` の `form` キーワード引数、SFG_DENY body
  の `form: <値>` 行
- **現状**: `bare`/`fd_prefixed`/`no_space`/`quoted` の 4 種を返して LLM に
  「どう書かれていたか」を伝える機能。0.5.0 で +36 テスト追加。
- **修正方針**:
  - `bash_deny(..., form=...)` の引数を削除
  - SFG_DENY body の `form:` 行は出さない (A3 と一体で SFG_DENY 自体を撤廃するなら同時)
- **回帰検査**:
...
### A3. `<SFG_DENY>` 構造化包装 (M4, 0.4.2) を plain text に戻す

- **対象**: `core/messages.py::_wrap_sfg_deny`, `_SFG_GUARD`, `BashDenyKind`,
  `EditDenyKind`, `bash_deny`, `edit_deny`, `policy_unavailable("deny")`、
  `redaction/sanitize.py::escape_xml_tag`
- **現状**: `<SFG_DENY tool reason guard>` で deny reason を構造化し、後段
...
- **回帰検査**:
  ...
  - 各 deny テストの `assertIn("<SFG_DENY")` を `assertIn(<key fragment>)` に置換
- **コード削減**: ≈80 行
- **Pri**: P1
...
- **現状**: 0.4.0 で `~/.claude/sensitive-files-guard/` を preferred とし、
  旧 `$XDG_CONFIG_HOME/sensitive-files-guard/` を fallback として試行する
...
- **コード追加**: ≈120 行
- **Pri**: P5
- **依存**: A3 (SFG_DENY 撤去) を先に行うとメッセージ整形が楽
...
- **現状**: 670 件
- **修正方針**:
  - A 系で約 220 件削除 (input_redirect 80, RedirectForm 36, SFG_DENY 12+7,
...
SFG_DENY 包装 (A3 撤去) の同時並行で衝突する」等の手戻りを回避する。
...
- **A7**: `patterns.local.txt` の参照先を `~/.claude/sensitive-files-guard/`
...
| P1 | A3 (`<SFG_DENY>` 構造化包装撤去) | 未着手 (PR 2) |
